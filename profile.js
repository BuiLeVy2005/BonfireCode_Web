...
const rankIcons = {
    'rank1': `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-full h-full"><path stroke-linecap="round" stroke-linejoin="round" d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"/></svg>`, // K? Luu Ðày
    'rank2': `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-full h-full"><path stroke-linecap="round" stroke-linejoin="round" d="M17.657 18.657A8 8 0 016.343 7.343S7 9 9 10c0-2 .5-5 2.986-7C14 5 16.09 5.777 17.656 7.343A7.975 7.975 0 0120 13a7.975 7.975 0 01-2.343 5.657z" /><path stroke-linecap="round" stroke-linejoin="round" d="M9.879 16.121A3 3 0 1012.015 11L11 14H9c0 .768.293 1.536.879 2.121z" /></svg>`, // K? Nhóm L?a
    'rank3': `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-full h-full"><path stroke-linecap="round" stroke-linejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>`, // K? Si Thu?t Toán
    'rank4': `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-full h-full"><path stroke-linecap="round" stroke-linejoin="round" d="M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2zM9 9h6v6H9V9z" /></svg>`, // Ma Tôn D? Li?u
...
function applyBanner(targetElement, bannerUrl) {
    ...
    if (bannerUrl.includes('.')) {
        targetElement.style.backgroundImage = `url('${getFullImageUrl(bannerUrl)}')`;
    } else {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (role === 'Admin') {
            ...
            if (dropdownMenu && !document.getElementById('admin-link')) {
                dropdownMenu.insertAdjacentHTML('afterbegin', '<a id="admin-link" href="admin.html" class="block px-4 py-1.5 text-[#ff4500] font-bold hover:bg-[#0366d6] hover:text-white transition"><i class="fa-solid fa-gavel mr-1"></i> Tòa Án T?i Cao</a>');
            }
        }
        ...
        // L?y thông tin th?c t? t? Database
        try {
            ...
            if (meRes.ok) {
                ...
                if (isMyProfile) {
                    ...
                    if (me.avatarUrl) {
                        ...
                        if (profileAvatar) profileAvatar.src = `${getFullImageUrl(me.avatarUrl)}`;
                    }
                    if (me.coverUrl) {
                        const profileCover = document.getElementById('profile-cover');
                        if (profileCover) profileCover.src = `${getFullImageUrl(me.coverUrl)}`;
                    }
                }
                
                // Luôn c?p nh?t Avatar trên thanh di?u hu?ng cho user dang dang nh?p
                ...
                if (btnAvatarDropdown) {
                    ...
                    if (navAvatar && me.avatarUrl) {
                        navAvatar.src = `${getFullImageUrl(me.avatarUrl)}`;
        ...
        console.error("L?i gi?i mã token:", e);
        window.location.href = 'auth.html';
        return;
    }
    ...
    try {
        ...
        if (!response.ok) throw new Error("Không th? t?i danh sách d? án");
        
        const allProjects = await response.json();
        
        // L?c d? án c?a user hi?n t?i (match theo authorName)
        ...
        try {
            ...
            if (profileRes.ok) {
                ...
                if (bioEl) bioEl.textContent = profileData.bio || 'Chua có ti?u s?.';
            ...
            console.error("L?i l?y thông tin followers:", e);
        }

        // 3. X? lý Logic Th?ng Kê
        await calculateAndRenderStats(userProjects);

        // 4. Render Ð? án N?i B?t (Pinned)
        renderPinnedProjects(userProjects, isMyProfile);
        
    } catch (error) {
        console.error("L?i khi fetch d? li?u d? án:", error);
    }
...
function fetchUserRank(profileData) {
    ...
    requiredEmbersEl.textContent = `/ ${nextRequired} Tàn L?a`;
    ...
    setTimeout(() => {
        if (progressBar) {
            ...
            if (percentage === 0) {
                ...
                // >= 80%: Th?c t?nh - Hi?u ?ng S?m sét (Ð? tía sang L?c lam di?n)
...
function setupEditModal(token) {
    ...
    if (btnEditProfile && editModal) {
        ...
        btnSaveEdit.addEventListener('click', async () => {
            ...
            if (!newFullName) {
                editError.textContent = "Tên hi?n th? không du?c d? tr?ng!";
                editError.classList.remove('hidden');
                return;
            }

            btnSaveEdit.disabled = true;
            btnSaveEdit.textContent = "Ðang luu...";
            editError.classList.add('hidden');

            try {
                // Gi?i mã token d? l?y UserId
                const payload = JSON.parse(decodeURIComponent(escape(atob(token.split('.')[1].replace(/-/g, '+').replace(/_/g, '/')))));
                ...
                const res = await fetch(endpoint, {
                    ...
                    headers: {
                        ...
                        // B? Content-Type d? trình duy?t t? set multipart/form-data
                    },
                    body: formData
                });
                ...
                if (!res.ok) {
                    ...
                    throw new Error(errorText || "API t? ch?i c?p nh?t h? so. Vui lòng ki?m tra l?i quy?n ho?c Backend.");
                }
                ...
                if (responseData.avatarUrl) {
                    const newAvatarSrc = `${getFullImageUrl(responseData.avatarUrl)}`;
                ...
                if (responseData.coverUrl) {
                    const newCoverSrc = `${getFullImageUrl(responseData.coverUrl)}`;
                    if (profileCoverEl) profileCoverEl.src = newCoverSrc;
                }

                btnSaveEdit.textContent = "Thành công!";                   
                // N?u không có json tr? v?, fallback b?ng file API FileReader n?u c?n
                if (inputAvatarFile && inputAvatarFile.files.length > 0) {
                ...
                // Hi?n th? Toast (T?m dùng alert, có th? tùy bi?n sau)
...
async function calculateAndRenderStats(projects) {
    ...
    // C?ng d?n di?m dánh giá và các ch? s? t? API
    await Promise.all(projects.map(async (p) => {
        try {
            ...
            if (rRes.ok) {
                ...
                totalStars += p.totalRatings; // Lu?t sao = t?ng s? lu?t dánh giá
            }
        } catch (e) {
            console.error("L?i l?y thông s? project:", p.id, e);
        }
    }));

    const statsHTML = `
        <div class="bg-[#161b22] border border-[#30363d] rounded-md p-4 text-center">
            <p class="text-xs text-gray-400 mb-1">T?ng d? án</p>
        ...
        <div class="bg-[#161b22] border border-[#30363d] rounded-md p-4 text-center">
            <p class="text-xs text-gray-400 mb-1">Bình lu?n</p>
    ...
    if (activityContainer && projects.length > 0) {
        ...
        recent.forEach(p => {
            ...
            // N?u có lu?t t?i ho?c comment/rating thì coi nhu là project cu, ngu?c l?i là "t?o m?i"
            ...
            <div class="flex gap-4 relative">
                ...
                <div class="pb-6">
                    ...
                    <p class="text-sm text-gray-300">
                        ${isNew ? 'Ðã t?o kho luu tr? m?i' : 'Ðã t?i lên d? án'} 
                        <a href="detail.html?id=${p.id}" class="text-blue-400 font-semibold hover:underline">${p.title}</a>
                    </p>
                    <p class="text-xs text-gray-500 mt-1 line-clamp-2">${p.description || 'Không có mô t?'}</p>
                </div>
            </div>
            `;
        });
        
        // Nút xem thêm n?u còn nhi?u ho?t d?ng
        if (projects.length > 3) {
            actHtml += `
            <div class="mt-4 text-center">
                <a href="#" class="text-blue-400 hover:underline text-sm font-semibold">Xem thêm ho?t d?ng...</a>
            </div>`;
        }

        activityContainer.innerHTML = actHtml;
    } else if (activityContainer) {
        activityContainer.innerHTML = `<p class="text-sm text-gray-500">Chua có ho?t d?ng nào.</p>`;
    }
}
...
function renderPinnedProjects(projects, isMyProfile = false) {
    ...
    if (projects.length === 0) {
        pinnedContainer.innerHTML = `<p class="text-sm text-gray-500 col-span-2">Chua có d? án nào du?c chia s?.</p>`;
        return;
    }

    // S?p x?p theo t?ng (lu?t sao + lu?t bình lu?n) gi?m d?n d? l?y các d? án n?i b?t
    const sorted = [...projects].sort((a, b) => {
        const scoreA = (a.totalRatings || 0) + (a.commentCount || 0);
        const scoreB = (b.totalRatings || 0) + (b.commentCount || 0);
        return scoreB - scoreA;
    });
    const pinned = sorted.slice(0, 2); // L?y t?i da 2 d? án n?i b?t
            ...
            <div class="p-4 flex-grow flex flex-col justify-between">
                <div>
                    <div class="flex items-center justify-between mb-2 pr-6">
                        ...
                        <span class="text-xs text-gray-400 border border-[#30363d] px-2 py-0.5 rounded-full min-w-max">Công c?ng</span>
                    </div>
                    <p class="text-xs text-gray-400 mb-4 h-10 line-clamp-2">${p.description || 'Chua có mô t?'}</p>
                </div>
...
// 6. X? lý Global Click cho Dropdown, Delete, Quick Edit
document.addEventListener('click', async (e) => {
    // Ðóng t?t c? dropdown n?u click ra ngoài
    ...
    if (e.target.closest('.btn-delete-proj')) {
        ...
        if(!confirm('B?n có ch?c ch?n mu?n hi?n t? (xóa) b?o v?t này không? Hành d?ng này không th? hoàn tác!')) return;
        ...
        try {
            ...
            if (!res.ok) throw new Error("API t? ch?i xóa");
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (modal && btnCancel && btnSave) {
        ...
        btnSave.addEventListener('click', async () => {
            ...
            if (!title) {
                errorBox.textContent = "Tên d? án là b?t bu?c.";
                errorBox.classList.remove('hidden');
                return;
            }
            
            btnSave.disabled = true;
            btnSave.textContent = "Ðang luu...";
            ...
            try {
                ...
                // ? dây API update project dùng JSON (PUT)
                // N?u backend c?n FormData cho update project, ta có th? d?i thành FormData. ? dây dùng JSON t?m theo m?u.
                const updateData = {
                    title: title,
                    description: desc,
                    categoryIds: catIds
                ...
                if (!res.ok) {
                    ...
                    throw new Error(text || "L?i c?p nh?t d? án");
                }
                
                alert("C?p nh?t d? án thành công!");
                modal.classList.add('hidden');
                
                // Reload danh sách project b?ng cách F5 ho?c fetch l?i (F5 cho ch?c)
...
function setupTabs() {
    ...
    if (tabMyProjects) {
        tabMyProjects.addEventListener('click', () => {
            ...
            renderProjectsList(userProjects, 'my-projects-grid'); // Dùng userProjects t? API ban d?u
        });
    }
...
function renderProjectsList(projects, containerId) {
    ...
    if (!projects || projects.length === 0) {
        container.innerHTML = '<p class="text-gray-400 col-span-full">Không có d? án nào d? hi?n th?.</p>';
        return;
    }

    projects.forEach(p => {
        const thumbUrl = p.thumbnailUrl ? (p.thumbnailUrl.startsWith('http') ? p.thumbnailUrl : `${getFullImageUrl(p.thumbnailUrl)}`) : 'https://placehold.co/600x400/21262d/8b949e?text=No+Image';
        ...
        card.innerHTML = `
            ...
            <div class="p-4 flex-grow flex flex-col cursor-pointer" onclick="window.location.href='detail.html?id=${p.id}'">
                ...
                <p class="text-sm text-gray-400 line-clamp-2 mb-4 flex-grow">${p.description || 'Chua có mô t?...'}</p>
                ...
                <div class="flex justify-between items-center text-xs pt-3 border-t border-[#30363d]">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex items-center text-[#8b949e]" title="Ði?m dánh giá trung bình">
                            ...
                            <span class="font-medium ${totalRatings > 0 ? 'text-purple-400' : 'text-[#8b949e]'}">
                                ${totalRatings > 0 ? `${averageRating} (${totalRatings})` : 'Chua có'}
                            </span>
                        </div>
                    </div>
                </div>
...
window.editProject = function(id) {
    ...
    const modal = document.getElementById('quickEditProjectModal');
    if (!modal) return alert("Không tìm th?y modal s?a.");
...
window.deleteProject = async function(id) {
    event.stopPropagation();
    if (!confirm("Ngài có ch?c ch?n mu?n xóa d? án này không?")) return;
...
async function loadLikedProjects() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (grid) grid.innerHTML = '<p class="text-red-400 col-span-full">Không th? t?i d? li?u. Hãy th? l?i.</p>';
        }
    } catch (e) {
        if (loading) loading.classList.add('hidden');
        console.error("L?i m?ng:", e);
        if (grid) grid.innerHTML = '<p class="text-red-400 col-span-full">L?i k?t n?i máy ch?.</p>';
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (btnOpenIdentity && modalIdentity) {
                ...
                document.querySelectorAll(selector).forEach(item => {
                    ...
                    if (userRankId < reqRank) {
                        ...
                        overlay.innerHTML = `
                            ...
                            <span class="text-[10px] text-[#d4af37] font-bold uppercase text-center leading-tight">Yêu c?u<br>${rankNames[reqRank]}</span>
                        `;
            ...
            inputUploadAvatar.addEventListener('change', async (e) => {
                ...
                try {
                    ...
                    if (res.ok) {
                        ...
                        if (prevAvatar) prevAvatar.src = `${getFullImageUrl(url)}`;
                        ...
                        if (avatarGrid) {
                            ...
                            newImg.src = `${getFullImageUrl(url)}`;
                        ...
                        alert(data.message || 'L?i t?i ?nh lên.');
                    }
                } catch (err) {
                    console.error(err);
                    alert('L?i h? th?ng.');
        ...
        document.querySelectorAll('.id-item-border').forEach(item => {
            item.addEventListener('click', (e) => {
                if (e.currentTarget.getAttribute('data-locked') === 'true') {
                    ...
                    alert('B?n c?n thang c?p d? m? khóa Khung này!');
                    return;
                }
                ...
                if (prevBorder) {
                    ...
                    if (svgElement) {
                        ...
                        prevBorder.src = `${getFullImageUrl(url)}`;
                        prevBorder.classList.remove('hidden');
                    } else {
                        // No border
                        prevBorder.classList.add('hidden');
        ...
        document.querySelectorAll('.id-item-banner').forEach(item => {
            item.addEventListener('click', (e) => {
                if (e.currentTarget.getAttribute('data-locked') === 'true') {
                    alert('B?n c?n thang c?p d? m? khóa C? này!');
                    return;
                }
